...
export function createRiskState(equity: number): RiskState {
  return { startEquity: equity, equity, peakEquity: equity, dailyPnl: 0, halted: false };
}
...
export function checkRisk(settings: Settings, state: RiskState, nextNotional: number): { ok: boolean; reason?: string } {
  ...
  if (nextNotional > settings.risk.maxNotionalUsd) {
    return { ok: false, reason: "max_notional" };
  }
  if (nextNotional > settings.risk.maxPositionUsd) {
    return { ok: false, reason: "max_position" };
  }
  return { ok: true };
}
