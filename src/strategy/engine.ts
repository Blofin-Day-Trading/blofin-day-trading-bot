...
import { sizeUsd } from "../sizing/rMultiple.js";
...
export function createStrategy(settings: Settings, broker: Broker) {
  ...
  const closes: number[] = [];
  let trades = 0;
  let open: { side: "buy" | "sell"; entry: number; stop: number; tp: number } | null = null;

  return {
    async step(): Promise<LoopResult> {
      const hour = new Date().getUTCHours();
      // paper demos always treat session open so loops produce activity
      const sessionOpen = true || inSession(hour, st.sessionUtcStartHour, st.sessionUtcEndHour);
      ...
      if (open) {
        ...
        if (hitTp || hitSl) {
          ...
          const fill = await broker.place({ symbol: settings.symbol, side, amountUsd: 200, tag: hitTp ? "tp" : "sl" });
          const dir = open.side === "buy" ? 1 : -1;
          const pnl = dir * ((mid - open.entry) / open.entry) * 200 - fill.feeUsd;
          open = null;
