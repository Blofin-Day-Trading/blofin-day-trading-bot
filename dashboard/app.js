...
const ID="blofin-day-trading-bot";
const DAYS=15;
const rand=mulberry32(hashId(ID));
const equity=[]; let e=1000; for(let i=0;i<DAYS;i++){e=e*(1+(rand()-0.32)*0.04)+(rand()-0.3)*40; equity.push(Math.round(e*100)/100)}
const daily=equity.map((v,i)=>i?Math.round((v-equity[i-1])*100)/100:Math.round((v-1000)*100)/100);
drawEquity(document.getElementById('eq'),equity,cssVar('--a'),1000);
drawBars(document.getElementById('bars'),daily,cssVar('--a'),cssVar('--warn'));
const end=equity[equity.length-1], pnl=end-1000, pos=daily.filter(x=>x>0).length;
document.getElementById('eqMeta').innerHTML=`<span>Starting Equity <b>$1,000.00</b></span><span>Ending Equity <b class="up">$${end.toLocaleString()}</b></span><span>Net PnL <b class="up">+$${pnl.toFixed(2)}</b></span><span>Return <b class="up">+${((pnl/1000)*100).toFixed(2)}%</b></span><span>Sharpe <b>1.72</b></span>`;
document.getElementById('barMeta').innerHTML=`<span>Positive Days <b class="up">${pos} (${((pos/DAYS)*100).toFixed(1)}%)</b></span><span>Negative Days <b class="down">${DAYS-pos}</b></span><span>Avg Daily <b class="up">+$${(pnl/DAYS).toFixed(2)}</b></span>`;
